...
- Hooks principais: `useIncrementalMarketData` (carrega candles via API com fallback mock), `useIndicators` (agora apoiado em helpers LLM-friendly como `utils/indicators/indicatorPaths.ts` e hooks dedicados em `hooks/indicators/`), `useStrategies`, `useBacktest` (mock local), `useLeanBacktest` (Lean), `useNormalizationSettings`, `useAvailableFrames`.
